...
To contribute to the existing climate studies of Botswana, this study presented a series of the new maps based on the open datasets. The \hl{maps} are performed through the command line, which increased the speed and accuracy of mapping and enabled to represent spatial data of various complexity. The \hl{general} objective was to \hl{map} the variability of environmental and climate parameters of the country.
...
\section{Study Area}
\subsection{Relief}
The topography of Botswana has the highest points along the south-east regions of the country, in the Kalahari Basin). In the rest of the regions, the landscapes are levelled, which creates unique climate setting. The territory of the country is covered by the diverse types of landscapes, including a complex mosaics of grass, sands and savanna \cite{Campbell}. The Okavango Delta is presented by the alluvial fan of uniform topographic gradient. 
